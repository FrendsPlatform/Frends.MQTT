...
/// <summary>
/// Helper class that connects to broker.
/// </summary>
internal class MQTTConnectionCreator
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return new ResultReceive(success: false, clientID: clientID, $"Error while trying to connect to MQTT topic: {cException.Message} InnerException: {cException.InnerException.Message}", messagesList: messagesList);
        }
        catch (Exception ex)
        {
            return new ResultReceive(success: false, clientID: clientID, $"Error while trying to connect to MQTT topic: {ex.Message} InnerException: {ex.InnerException.InnerException.Message}", messagesList: messagesList);
        }

        if (connectionResponse.ResultCode != MqttClientConnectResultCode.Success)
        {
            return new ResultReceive(success: false, clientID: clientID, $"Broker rejected connection: {connectionResponse.ResultCode}. ReasonString: {connectionResponse.ReasonString ?? "none provided"}. {disconnectInfo ?? string.Empty}", messagesList: messagesList);
        }
